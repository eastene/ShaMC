...
void ShaMC::fit(SharedDataset &X) {
    ...
    for (i = 0; i < parameters.maxiter; i++) {
        ...
        for (const auto &centroid : mediods) {
            Info sharedInfo;
#pragma omp parallel for schedule(dynamic) shared(centroid, parameters, minPoints, currentSize, clusterCount, failedAttempts, i, sharedInfo) private(me)
            for (me = 0; me < omp_get_num_threads(); me++) {
                ...
                fpm.Mine_Patterns_Parallel(transactions, 1, 2, &sharedInfo);
            }
        }
